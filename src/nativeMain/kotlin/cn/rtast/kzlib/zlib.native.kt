...
public actual fun ByteArray.gzipCompress(): ByteArray = memScoped {
    val stream = alloc<z_stream>()
    stream.zalloc = null
    stream.zfree = null
    stream.opaque = null
    deflateInit2_(
        stream.ptr, Z_DEFAULT_COMPRESSION, Z_DEFLATED,
        16 + MAX_WBITS, 8, Z_DEFAULT_STRATEGY, ZLIB_VERSION, sizeOf<z_stream>().toInt()
    )
    stream.next_in = this@gzipCompress.refTo(0).getPointer(this).reinterpret()
    stream.avail_in = this@gzipCompress.size.toUInt()
...
public actual fun ByteArray.gzipDecompress(): ByteArray = memScoped {
    val stream = alloc<z_stream>()
    stream.zalloc = null
    stream.zfree = null
    stream.opaque = null
    check(inflateInit2_(stream.ptr, 16 + MAX_WBITS, ZLIB_VERSION, sizeOf<z_stream>().toInt()) == Z_OK)
    stream.next_in = this@gzipDecompress.refTo(0).getPointer(this).reinterpret()
    stream.avail_in = this@gzipDecompress.size.toUInt()
    val output = mutableListOf<Byte>()
    val bufferSize = 4096
